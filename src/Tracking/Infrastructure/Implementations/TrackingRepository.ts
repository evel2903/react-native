...
class TrackingRepository implements ITrackingRepository {
    ...
    public async getTrackingByLocation(
        ...
        try {
            ...
            if (!response.data || !Array.isArray(response.data)) {
                throw new Error(
                    'Unexpected API response format for location tracking'
                )
            }
    ...
    public async getTrackingByGoods(
        goodsCode: string
    ): Promise<GoodsTrackingEntity[]> {
        try {
            const response: any = await this.httpClient.get(
            ...
            if (!response.data || !Array.isArray(response.data)) {
                throw new Error(
                    'Unexpected API response format for goods tracking'
                )
            }
