...
export class TrackingStore implements TrackingStoreState {
    ...
    async trackByLocation(code: string) {
        try {
            ...
            if (!params) {
                throw new Error(
                    'Invalid location format. Expected format: SHELF_CODE/LEVEL/POSITION'
                )
            }

            this.locationTrackingData =
                await this.getLocationTrackingUseCase.execute(params)
    ...
    async trackByGoods(goodsCode: string) {
        try {
            ...
            this.goodsTrackingData = await this.getGoodsTrackingUseCase.execute(
                goodsCode
            )
