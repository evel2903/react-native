...
@injectable()
export class GetCurrentApprovalStageUseCase implements UseCase<GetCurrentApprovalStagePayload, Promise<ApprovalStage>> {
    constructor(
        @inject(IStockInRepositoryToken)
        private readonly stockInRepository: IStockInRepository
    ) {}

    public execute(payload: GetCurrentApprovalStagePayload): Promise<ApprovalStage> {
